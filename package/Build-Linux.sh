...
# Build SeedKeeper Debian/Ubuntu binary package release


if ! (cat /etc/os-release | grep -E "20\.04(\.[0-9]+)? LTS \(Focal Fossa\)" > /dev/null ) then
  echo "This SeedKeeper building script only runs on Ubuntu 20.04."
  exit 1
fi
...
python -OO -m PyInstaller package/seedkeeper.spec
deactivate
